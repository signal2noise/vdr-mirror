...
#define PROGBARS 1
       ...
       if (isprogress) {
         ...
         int xt = x3 + 5 + Tab(i);
         int w = cFont::GetFont(fontOsd)->Width(s);
         int fh = cFont::GetFont(fontOsd)->Height();
         
         osd->DrawRectangle(xt, y, xt + w, y + fh, ColorBg);                        /* background   */
         
         /* border width */
          #define bw 2 

#ifdef PROGBAR_BORDERS
         osd->DrawRectangle(xt, y , xt + w, y + bw, ColorFg);                       /* lower border */
         osd->DrawRectangle(xt, y + bw, xt + bw, y + fh - bw, ColorFg);             /* left border  */
         osd->DrawRectangle(xt, y + fh - 2*bw, xt + w, y + fh - bw, ColorFg);       /* upper border */
#endif
#define RIGHT_BORDER 1
#ifdef RIGHT_BORDER
         osd->DrawRectangle(xt + w - bw, y + bw, xt + w, y + fh - 2*bw, ColorFg);   /* right border */
#endif
         
         int border = current*(w-2*bw)/total;
         osd->DrawRectangle(xt+2*bw, y + 8, xt + border, y + fh - 10, ColorFg);
