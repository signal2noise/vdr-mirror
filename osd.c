...
void cBitmap::DrawText(int x, int y, const char *s, tColor ColorFg, tColor ColorBg, const cFont *Font, int Width, int Height, int Alignment)
{
  if (bitmap) {
              ...
              for (int row = 0; row < h; row++) {
                  ...
                  for (int col = CharData->width; col-- > 0; ) {
                      if (ColorBg != clrTransparent || (PixelData & 1))
                         SetIndexFast(x + col, y + row, (PixelData & 1) ? fg : bg);
                      PixelData >>= 1;
                      }
                  }
              }
     ...
     UpdateDirty(startx,starty,x-1,starty+h-1);
     }
}
...
cOsd *cOsdProvider::CreateTrueColorOsd(int Left, int Top) // GT: True color support.
{
    return CreateOsd(Left, Top);
}
...
cOsd *cOsdProvider::NewTrueColorOsd(int Left, int Top, int Random, bool dontHide) // GT: True color osd support.
{
    if (Random) {
  ...
  if (cOsd::IsOpen())
     ...
     return osdProvider->CreateTrueColorOsd(Left, Top);
  else
     esyslog("ERROR: no OSD provider available - using dummy OSD!");
  return new cOsd(Left, Top); // create a dummy cOsd, so that access won't result in a segfault
}
